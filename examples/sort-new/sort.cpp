...
int main(int argc, char** argv)
{
	...
	//argv[1] = "6";  // Problem
	//argv[2] = "1";  // Searcher
	//argv[3] = "3"; // Number of iteration (In DLNezarat it is equal the top N configuration which should be retreive)
	//argv[4] = "1"; // Number of expriment
	//argv[5] = "F:\\GPU-1070";
	//argv[6] = "7";
	//argv[7] = "2";
	...
	if (argc > 8) {
		...
		dir = std::string(argv[5]);
		if (argc == 8) {
			hw = std::stoul(std::string(argv[6]));
			arch = std::stoul(std::string(argv[7]));
			}
	...
	for (size_t ex = 0; ex < experiments; ex++)
	{
		...
		//tuner.printResult(kernelId, std::cout, ktt::PrintFormat::CSV);
	...
	while (std::getline(file, read_str)) {
		std::cout << "Computation time for best configuration for Deep Learning searcher is :" << read_str << std::endl;
	}
	file.close();*/
...
void printSearcherName(eSearchMethod searchMethod) {
	...
	case RANDOM:
		...
		break;
	case DLNezarat:
		std::cout << "Deep Learning Network (Sensitive to Hardware model)";
		break;
	case ANNEALING:
...
void configureSearcher(ktt::Tuner &tuner, eSearchMethod searcher, eProblem problem) {
	...
	case DLNezarat:{
		double problemNum = problem;
		tuner.setSearchMethod(ktt::SearchMethod::DLNezarat, { problemNum, hw, arch }); // Arguments are problem, Hardware and Architecture code respectively
		break;
	}
	case ANNEALING: {
